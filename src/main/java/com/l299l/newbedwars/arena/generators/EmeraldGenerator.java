...
public class EmeraldGenerator implements Generator {
    ...
    private final GeneratorType type = GeneratorType.EMERALD;
    ...
    public EmeraldGenerator(Location location) {
        ...
        maxLevel = generatorLeveling.getEmeraldGeneratorSettings().getMaxLevel();
        upgrade();
    }

    @Override
    public void start() {
        hologram = new Hologram(NewBedwars.plugin.getMessages().getMsgToConsole("EmeraldGenerator"), location.clone(),
        ...
        task = new BukkitRunnable() {
            ...
            public void run() {
                if (delay[0] == 0) {
                    ...
                    location.getWorld().dropItem(location.clone().add(0, 0.5, 0), new ItemStack(Material.EMERALD, Math.max(1, emeraldSpawnAmount)));
                } else {
                    delay[0]--;
                }
    ...
    public boolean upgrade() {
        ...
        GeneratorLeveling generatorLeveling = arena.getGeneratorsLeveling();
        emeraldSpawnRate = generatorLeveling.getEmeraldGeneratorSettings().getEmeraldSpawnRate().get(level - 1);
        emeraldSpawnAmount = generatorLeveling.getEmeraldGeneratorSettings().getEmeraldSpawnAmount().get(level - 1);
        spawnDelay = generatorLeveling.getEmeraldGeneratorSettings().getSpawnDelay().get(level - 1);
        return true;
    }
