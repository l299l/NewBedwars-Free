...
public class SetWaitingPos2Command extends SubCommand {
    private final Messages msg;

    public SetWaitingPos2Command() {
        this.msg = NewBedwars.plugin.getMessages();
    }
    @Override
    public String getName() {
        return "setWaitingPos2";
    }

    @Override
    public String getDescription() {
        return "Command to set second position of waiting region.";
    }

    @Override
    public String getSyntax() {
        return "/bw waitingPos2 (<x> <y> <z>)";
    }
    ...
    public void perform(Player player, String[] args, IArena arena) {
        if(args.length == 4) {
            try {
                ...
                arena.setWaitingPos2(loc);
                player.sendMessage(msg.getMsg(player, "WaitingPos2Set") + arena.getWaitingPos2().getBlockX() + " " + arena.getWaitingPos2().getBlockY() + " " + arena.getWaitingPos2().getBlockZ());
            }catch (Exception e) {
